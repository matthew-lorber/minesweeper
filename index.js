...
$("#sizeSelect").click(()=>{
    ...
    if (size === 10) {numBombs = 10}; 
    if (size === 20) {numBombs = 40};
    if (size === 25) {numBombs = 99};
    
    // PLACE RANDOM BOMBS IN ARRAY
    let bombs = [];
    let j=0;
    while (j < numBombs) {
        let bomb = [Math.floor(Math.random() * size), Math.floor(Math.random() * size)];
    ...
    $(".cell").mouseup(function(e){
        ...
        $("#faceImg").attr("src","./none.png"); // change face back
        ...
        if (e.which === 1) {
            ...
            switch (this.innerHTML) {
                case "*": // bomb clicked: end the game
                    $(".bomb").html("💣").css("color","white"); // add a bomb emoji
                    clearInterval(timer); // stop the timer
                    $(".cell").off("mousedown").off("mouseup").off("contextmenu"); // disable functions except for game reset (faceImg click)
                    $("#faceImg").attr("src","./dead.png");
                    break;
                case "":
    ...
    // RESET BUTTON
    $("#faceImg").click(()=>location.reload()); // resets the game (back to difficult selection)
});